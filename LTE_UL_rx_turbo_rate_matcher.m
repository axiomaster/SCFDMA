function dk = LTE_UL_rx_turbo_rate_matcher(LTE_params,ek,UE_signaling)
stream_index=1;
...
N_IR = floor(LTE_params.N_soft / (K_MIMO*min(LTE_params.HARQ_processes,M_limit)));
...
for i=1:C
    ...
    wk{i} = LTE_common_turbo_rate_matching_bit_selection_and_pruning(double(ek),...
...
dk = subblock_interleaver(LTE_params,1,UE_signaling.turbo_rate_matcher_UL(stream_index),vk);
end
